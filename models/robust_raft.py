...
from models.update import BasicMultiUpdateBlock
from models.extractor import MatchingDecoder, MultiBasicEncoder, ResidualBlock, MultiVFMDecoder
from models.corr import CorrBlock1D, PytorchAlternateCorrBlock1D, CorrBlockFast1D, AlternateCorrBlock
from utils.utils import coords_grid, upflow8
from models.depthanything_v2.dpt import Feature
import matplotlib.pyplot as plt
from models.components import AntiAdverseFEBlock
...
class Robust_RAFT(nn.Module):
    def __init__(self, args):
        ...
        self.enhancement = args.enhancement
        
        self.model_size = 'base'
        self.model_path = '/data/ywang/my_projects/unimatch/pretrained/depth_anything_vitb14.pth'

        if self.args.vfm_type == 'dam':
            from models.depthanything.dpt import Feature
        elif self.args.vfm_type == 'damv2':
            from models.depthanything_v2.dpt import Feature
        elif self.args.vfm_type == 'sam':
            from models.segment_anything.build_sam import Feature
            
        self.backbone = Feature(self.model_size, self.model_path)
        
        if self.enhancement:
            self.enhance_block = AntiAdverseFEBlock(256, 256)

        self.cnet = MultiBasicEncoder(input_dim=args.VFM_dims, output_dim=[args.hidden_dims, context_dims], norm_fn=args.context_norm, downsample=args.n_downsample)
    ...
    def norm_fea(self, x):
        x = torch.sqrt(torch.sum(torch.pow(x, 2), dim=1))
        return x
    ...
    def forward(self, image1, image2, iters=16, clear=False, flow_init=None, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision):
            if self.args.shared_backbone:
                *cnet_list, x = self.cnet(torch.cat((image1, image2), dim=0), dual_inp=True, num_layers=self.args.n_gru_layers)
                ...
                cnet_list = self.cnet(left_vfm_feature_output, image1)
                fmap1, fmap2 = self.fnet(VFM_features[-1], [image1, image2])
                
                if self.enhancement and not clear:
                    fmap1 = self.enhance_block(fmap1)
                    fmap2 = self.enhance_block(fmap2)
            
            outputs.update({'fmap1': fmap1})
            outputs.update({'fmap2': fmap2})
            
                # fmap1, fmap2 = fmaps.split(dim=0, split_size=b)
