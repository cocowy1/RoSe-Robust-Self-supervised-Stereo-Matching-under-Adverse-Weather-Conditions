...
class AntiAdverseFEBlock(nn.Module):
    def __init__(self, in_channels, output_channels):
        super(AntiAdverseFEBlock, self).__init__()
        self.dnc_block_combined = DNCBlock_combined(in_channels) 
        self.fgm_block = FGMBlock(in_channels)  
        self.conv_layer = nn.Sequential(nn.Conv2d(2*in_channels, in_channels, kernel_size=3, padding=1, bias=False),
                                        nn.InstanceNorm2d(in_channels),
                                        nn.ReLU(inplace=True),
                                        nn.Conv2d(in_channels, output_channels, kernel_size=3, padding=1, bias=False),)

    def forward(self, x_ori, clear=True):
        # if not clear:
        
        x = self.dnc_block_combined(x_ori)
        x = self.fgm_block(x)                
...
class FGMBlock(nn.Module):
    def __init__(self, vit_dim):
        ...
        self.conv_layer = nn.Conv2d(self.num_channels, self.num_channels, kernel_size=1, bias=False)

    def forward(self, x):
        fft_map = torch.fft.fftn(x, dim=(-2, -1))
        
        magnitude_map = torch.abs(fft_map)
        phase_map = torch.angle(fft_map)
        
        # magnitude_map = self.conv_layer(magnitude_map)
        
        real_part = magnitude_map * torch.cos(phase_map)
        imag_part = magnitude_map * torch.sin(phase_map)
        modified_fft_map = torch.complex(real_part, imag_part)

        reconstructed_x = torch.real(torch.fft.ifftn(modified_fft_map, dim=(-2, -1)))
