...
class DPT_DINOv2(nn.Module):
    def __init__(self, encoder='vitl', features=256, out_channels=[256, 512, 1024, 1024], use_bn=False, use_clstoken=False, localhub=True):
        ...
        if localhub:
            self.pretrained = torch.hub.load('models/torchhub/facebookresearch_dinov2_main', 'dinov2_{:}14'.format(encoder), source='local', pretrained=True)
        else:
...
# model = DepthAnything(model_configs[encoder]).cuda().eval()
# model.load_state_dict(torch.load(f'/home/ywang/my_projects/others/CFNet-foundation/pretrained/dam/depth_anything_{encoder}14.pth'))
# print(model)


class dam_small(DPT_DINOv2):
    def __init__(self, **kwargs):
        super(dam_small, self).__init__(
           encoder='vits', features=64, out_channels=[48, 96, 192, 384], use_bn=False, use_clstoken=False,)

class dam_base(DPT_DINOv2):
    def __init__(self, **kwargs):
        super(dam_base, self).__init__(
           encoder='vitb', features=128, out_channels=[96, 192, 384, 768], use_bn=False, use_clstoken=False,)

class dam_large(DPT_DINOv2):
    def __init__(self, **kwargs):
        super(dam_large, self).__init__(
...
class Feature(SubModule):
    def __init__(self, model_name, model_path=None):
        ...
        if model_name == 'base':
            self.model = dam_base()
            model_path = 'pretrained/dptv2/depth_anything_v2_vitb.pth'
