...
class ImageEncoderViT(nn.Module):
    ...
    def interpolate_position_embed(self, pos_embed, img_size):
        embedding_size = pos_embed.shape[-1]
        # num_patches = 1024
        num_extra_tokens = 1
        # height (== width) for the checkpoint position embedding
        orig_size = int((pos_embed.shape[-2] - num_extra_tokens) ** 0.5)
        # if orig_size*orig_size != img_size[0]*img_size[1]:
        #     print(
        #         "Position interpolate from %dx%d to %dx%d"
        #         % (orig_size, orig_size, img_size[0], img_size[1])
        #     )
        # only the position tokens are interpolated
        pos_tokens = pos_embed.permute(0, 3, 1, 2)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x, img_size = self.patch_embed(x)

        features = []
        if self.pos_embed is not None:
            new_pos_embed= self.interpolate_position_embed(self.pos_embed, img_size)
            x = x + new_pos_embed
