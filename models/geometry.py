...
def coords_grid(b, h, w, homogeneous=False, device=None):
    y, x = torch.meshgrid([torch.arange(0, h, dtype=torch.float32, device=device),
                               torch.arange(0, w, dtype=torch.float32, device=device)], indexing='ij')
...
def generate_window_grid(h_min, h_max, w_min, w_max, len_h, len_w, device=None):
    ...
    x, y = torch.meshgrid([torch.linspace(w_min, w_max, len_w, dtype=torch.float32, device=device),
                           torch.linspace(h_min, h_max, len_h, dtype=torch.float32, device=device)],
                           indexing='ij')
    grid = torch.stack((x, y), -1).transpose(0, 1).contiguous().float()  # [H, W, 2]
...
def bilinear_sample(img, sample_coords, mode='bilinear', padding_mode='zeros', return_mask=False):
    ...
    if sample_coords.size(1) != 2:  # [B, H, W, 2]
        sample_coords = sample_coords.permute(0, 3, 1, 2).contiguous() 
