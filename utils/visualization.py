...
def save_images(logger, mode_tag, images_dict, global_step, img_path=None):
    ...
    for tag, values in images_dict.items():
        ...
        for idx, value in enumerate(values):
            ...
            if img_path:
                if "pred_disp" in tag:
                    pred_disp = value.squeeze().detach().cpu().numpy()
                    pred_disp = np.round(pred_disp * 256).astype(np.uint16)
                    skimage.io.imsave(img_path + tag + str(global_step) + '.png', pred_disp)
