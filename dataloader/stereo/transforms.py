...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        left_ori = np.transpose(sample['left_ori'], (2, 0, 1))  # [3, H, W]
        right_ori = np.transpose(sample['right_ori'], (2, 0, 1))
        
        if self.normalize:
            sample['left_ori'] = torch.from_numpy(left_ori) / 255.
        else:
            sample['left_ori'] = torch.from_numpy(left_ori) 

        if self.normalize:
            sample['right_ori'] = torch.from_numpy(right_ori) / 255.
        else:
            sample['right_ori'] = torch.from_numpy(right_ori) 
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        if 'left' in sample.keys():
            norm_keys = ['left', 'right', 'left_ori', 'right_ori']
        
        else:
            norm_keys = ['left_ori', 'right_ori']
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ori_height, ori_width = sample['left_ori'].shape[:2]
        ...
        if self.img_height > ori_height or self.img_width > ori_width:
            ...
            sample['left_ori'] = np.lib.pad(sample['left_ori'],
                                        ((top_pad, 0), (0, right_pad), (0, 0)),
                                        mode='edge')
            sample['right_ori'] = np.lib.pad(sample['right_ori'],
                                         ((top_pad, 0), (0, right_pad), (0, 0)),
                                         mode='edge')
        ...
        sample['left_ori'] = self.crop_img(sample['left_ori'])
        sample['right_ori'] = self.crop_img(sample['right_ori'])
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            ...
            if 'left_ori' in sample.keys():
                sample['left_ori'] = np.copy(np.flipud(sample['left_ori']))
                sample['right_ori'] = np.copy(np.flipud(sample['right_ori']))
...
class ToPILImage(object):
    ...
    def __call__(self, sample):
        ...
        sample['left_ori'] = Image.fromarray(sample['left_ori'].astype('uint8'))
        sample['right_ori'] = Image.fromarray(sample['right_ori'].astype('uint8'))
...
class ToNumpyArray(object):
    def __call__(self, sample):
        ...
        sample['left_ori'] = np.array(sample['left_ori']).astype(np.float32)
        sample['right_ori'] = np.array(sample['right_ori']).astype(np.float32)
...
class RandomGamma(object):
    ...
    def __call__(self, sample):
        ...
        if np.random.random() < 0.5:
            gamma = np.random.uniform(0.7, 1.5, 2)  # adopted from FlowNet
...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        if 'left' in sample.keys():
            ...
            if 'pseudo_disp' in sample.keys():
                sample['pseudo_disp'] = cv2.resize(
                sample['pseudo_disp'], None, fx=scale_x, fy=scale_y,
                interpolation=cv2.INTER_LINEAR if not self.nearest_interp else cv2.INTER_NEAREST
            ) * scale_x
...
class RandomMaskOcclusion(object):
    ...
    def __call__(self, sample):
        if 'left' in sample.keys():
            ...
            mshape = round(ht / self.mask_block_size), \
                round(wd / self.mask_block_size), 1
            input_mask = np.random.rand(mshape[0], mshape[1], mshape[2])
            input_mask = (input_mask > self.mask_ratio).astype(float)
            input_mask = cv2.resize(input_mask, (wd, ht), interpolation=cv2.INTER_LINEAR)
            input_mask[input_mask<0.5] = 0.
            input_mask[input_mask>=0.5] = 1.
            input_mask = np.expand_dims(input_mask, axis=-1)
            sample['left'] = img1 * input_mask
        else:
            pass
